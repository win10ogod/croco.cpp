...
ggml_tensor * llm_graph_context::build_moe_ffn(
    ...
    // ggml_tensor * par = ggml_mul(ctx0, up, cur); // [n_ff, n_expert_used, n_tokens] // OLD


    // This is equivalent to the commented out code below

    // ggml_tensor * parent = ggml_fused_mul_unary(ctx0, cur, up, type_op == LLM_FFN_SILU ? GGML_UNARY_OP_SILU : GGML_UNARY_OP_GELU); // IK
    ...
    // if (gate_exps) { // NEW
        // cur = ggml_fused_mul_unary(ctx0, cur, up, type_op == LLM_FFN_SILU ? GGML_GLU_OP_SWIGLU : GGML_GLU_OP_GEGLU); // [n_ff, n_expert_used, n_tokens]
        // cb(cur, "ffn_moe_gate_par", il);
    // }
    ...
    if (hparams.n_expert_used == 1) {
        return ggml_cont(ctx0, ggml_view_2d(ctx0, experts, n_embd, n_tokens, experts->nb[2], 0));
    }
    if (hparams.n_expert_used == 2) {
        return ggml_add(ctx0, ggml_view_2d(ctx0, experts, n_embd, n_tokens, experts->nb[2], 0),
                             ggml_view_2d(ctx0, experts, n_embd, n_tokens, experts->nb[2], experts->nb[1]));
    }
    return ggml_multi_add(ctx0, ggml_view_2d(ctx0, experts, n_embd, n_tokens, experts->nb[2], 0), n_expert_used);

/*     ggml_tensor * cur_experts[LLAMA_MAX_EXPERTS] = { nullptr }; // most recent
...
/*     if (n_expert_used >= 3) {
        aggregate experts // Croco wrong side attempt before I understood lol
